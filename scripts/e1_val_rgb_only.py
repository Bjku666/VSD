#!/usr/bin/env python3
"""评估 YOLO11n 的 RGB-only baseline，并导出完整指标与可视化结果。
...
def _render_required_metric_figures(out_dir: Path, required: Dict[str, Any]) -> List[Path]:
    ...
    plt.ylim(0.0, 1.0)
    plt.title("RGB-only Baseline Metric Overview")
    plt.ylabel("Score")
...
def _format_md(summary: Dict[str, Any]) -> str:
    ...
    lines = [
        "# YOLO11n RGB-only Baseline 指标报告",
        "",
        "## 标准指标",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="评估 YOLO11n RGB-only baseline 并导出完整指标。")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="评测输出目录；默认自动按实验名写入 experiments/e1_rgb_only/<run_name>/<split>",
    )
    parser.add_argument(
        "--ultra-project",
        type=str,
    ...
    if args.out_dir.strip():
        ...
        out_dir = Path("/mnt/disk2/lhr/VSD/experiments/e1_rgb_only") / run_name / args.split
