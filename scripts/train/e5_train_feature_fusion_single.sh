...
DEVICE="${2:-0,1}"
BATCH="${3:-64}"
RUN_NAME="${4:-yolo11n_e5_rgb_ir_640_ddp}"
WORKERS="${5:-16}"
MODE="${6:-bg}"
...
export VSD_E5_SCRIPTS_DIR="/mnt/disk2/lhr/VSD/scripts"
export PYTHONPATH="${VSD_E5_SCRIPTS_DIR}:${PYTHONPATH:-}"

echo "[train-e5] fusion_mode=${FUSION_MODE} device=${DEVICE} batch=${BATCH} workers=${WORKERS} run_name=${RUN_NAME}" | tee -a "${LOG_FILE}"
echo "[train-e5] log_file=${LOG_FILE}" | tee -a "${LOG_FILE}"

python /mnt/disk2/lhr/VSD/scripts/e5_train_feature_fusion_single.py \
    --mode "${FUSION_MODE}" \
    --device "${DEVICE}" \
    --batch "${BATCH}" \
    --workers "${WORKERS}" \
