#!/usr/bin/env python3
"""E5: dual-backbone single-layer feature fusion for YOLO11n.

This module provides:
1) E5SingleLayerFusionModel: RGB/IR dual-backbone with one fusion point (concat + 1x1 conv).
2) PairedYOLODataset: strict RGB/IR synchronized augmentation wrapper.
3) E5DetectionTrainer: minimal-intrusion trainer extension for rgb/ir/rgb_ir modes.
"""
...
class PairedYOLODataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        # Save RNG states so RGB and IR get identical stochastic transforms.
...
class E5SingleLayerFusionModel(DetectionModel):
    """YOLO11n dual-backbone model with one fusion point before neck."""
    ...
    def __init__(
        ...
        feat_channels = self._infer_backbone_out_channels()
        self.fusion_conv = Conv(feat_channels * 2, feat_channels, k=1, s=1)
        self._init_fusion_as_average(feat_channels)
    ...
    def _infer_backbone_out_channels(self) -> int:
        with torch.no_grad():
    ...
    def _init_fusion_as_average(self, channels: int) -> None:
        conv = self.fusion_conv.conv
        with torch.no_grad():
            conv.weight.zero_()
    ...
    def _run_backbone_with_storage(self, x: torch.Tensor, use_ir: bool, y: list[Any]) -> torch.Tensor:
    ...
    def predict(self, x, profile: bool = False, visualize: bool = False, augment: bool = False, embed=None):
        ...
        if self.fusion_mode == "rgb":
            ...
            x_rgb = x[:, :3, :, :]
            x_rgb = self._run_backbone_with_storage(x_rgb, use_ir=False, y=y)
        ...
        if self.fusion_mode == "ir":
            ...
            x_ir = self._run_backbone_with_storage(x_ir, use_ir=True, y=y)
        ...
        rgb_top = self._run_backbone_with_storage(x_rgb, use_ir=False, y=y)
        ir_top = self._run_backbone_with_storage(x_ir, use_ir=True, y=[None] * self.backbone_end)
        fused_top = self.fusion_conv(torch.cat((rgb_top, ir_top), dim=1))
        y[self.backbone_last_idx] = fused_top

        return self._run_shared_neck_head(fused_top, y)


class E5DetectionTrainer(DetectionTrainer):
    """DetectionTrainer extension for E5 fusion modes with minimal framework changes."""
    ...
    def get_model(self, cfg: str | dict | None = None, weights: str | nn.Module | None = None, verbose: bool = True):
        model = E5SingleLayerFusionModel(
