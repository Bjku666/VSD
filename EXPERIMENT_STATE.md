...
`S7_1a` 已于 2026-05-29 完成 full re-inference、统一验证、object-level、prediction export 与 gate review，结论为 `not_candidate`。当前第一优先方向已切换为 `S7_3a: Evidential reliability fusion lite (P4/P5-only)`。在任何 `S7-A` 单项或组合通过 gate 前，不启动 `E24_full`、`E15/E16/E19`、`E18_full`、`E20_full` 或 `E21`。
...
| E13_3b | done_single_seed_signal_only | E13_loss_check done / E13_2b reviewed | results/val/e13_3b_tiny_dark_center_loss_val/ | partial | small-scoped center-aware loss；单 seed mAP50-95=0.635858，AP_dark-small=0.513947，AP_tiny=0.558752，AP_low-contrast=0.637488，FP/image=1.285909，FPPI_dark=2.090909；只有单 seed / 低误报信号，corrected multi-seed 与 object-level gate 下尚未形成稳定候选 |
...
| E25_0 | done_not_candidate | E13_3b config / corrected seedfix only | results/S6_5_reliability_calibration/seedfix_e25_0_e13_3b_corrected_aggregate/ | no | corrected seedfix seeds 42/43/44 aggregate：mAP50-95=0.635443±0.001448，AP_dark-small=0.520173±0.001811，AP_dark-small_object=0.097365±0.002102，AP_tiny_object=0.052771±0.000811，AP_low-contrast_object=0.242618±0.004085，FP/image=1.724302±0.057282，FPPI_dark=3.063447±0.091391；object-level AP 未保住且误报高于 E6，不作为候选。旧 seed42/43/44 产物已判定 seed_pipeline_failed 并删除，不得使用 |
| E25_1 | done_limited_cached_predictions | E6 cached train/val predictions | results/val/e25_1_e6_calibration_sweep/ | partial | preliminary：基于 E20 缓存 E6 预测做离线校准；最佳全局 conf=0.40，val FP/image=1.236896、FPPI_dark=1.934659、class_confusion FP=739；NMS 仅限缓存 IoU=0.70，object AP 图使用 cached dark-small object AP50 proxy；不是正式实验闭环 |
...
| E26_2a | failed_train_split_source_unverified | class_confusion source CSV split check | results/S6_5_reliability_calibration/e26_2a_class_confusion_cls125_val/ | no | failed audit sample only：class_confusion 来源 CSV 的 split 字段为空，未通过 train-only source 校验。训练、统一验证、object-level 和 prediction export 虽已完成，但不得参与有效比较、候选判断或论文结论 |
| E26_2a_run_b48 | failed_train_split_source_unverified | class_confusion source CSV split check | results/S6_5_reliability_calibration/e26_2a_class_confusion_cls125/ | no | 同 E26_2a：高吞吐训练产物仅作为失败审计样本保留，不计入有效推进 |
| E26_2b | failed_train_split_source_unverified | class_confusion source CSV split check | results/S6_5_reliability_calibration/e26_2b_class_confusion_cls150_val/ | no | failed audit sample only：class_confusion 来源 CSV 的 split 字段为空，未通过 train-only source 校验。训练、统一验证、object-level 和 prediction export 虽已完成，但不得参与有效比较、候选判断或论文结论 |
...
| E27_1_full | done_not_candidate | E25_1_full complete / E26_1_full reviewed | results/S6_5_reliability_calibration/e27_1_full_metadata_verifier/ | no | full re-inference verifier completed；holdout AUC=0.967809，best score_final threshold=0.16，FP/image=1.230088，FPPI_dark=1.923295，FPPI_low-contrast=1.395997，但 post-calibration object-level AP_dark-small=0.066380、AP_tiny=0.037329、AP_low-contrast=0.215091，未通过 gate |
...
- E13_3b 只有单 seed / 低误报信号；corrected multi-seed 与 object-level gate 下尚未形成稳定候选。旧 E18/E25_0 多 seed 结果保持 invalid，不得用于 mean/std、候选判断或论文结论。
- S6 review：E23 object-level evaluator 已补齐，E22_1 已输出 per-taxonomy hard negative list；旧 E18_check / E18_6 审计结论已被 corrected seedfix 收口替代，不再作为继续重跑依据。E13_3b-light 已完成但 FP 显著升高且 object-level AP_dark-small 低于 E6，不作为候选；E14_1/E14_2/E14_3 CEBS 已完成 image-level 与 object-level，均未保住 dark-small object AP，不作为候选；E22_2a/E22_2b train-split background_far HN 已完成，但 FP 和 FPPI_dark 升高、object-level AP_dark-small 低于 E6，不作为候选；E14_4 无执行依据，E24_0 因无有效候选而阻塞。
- S6.5 复核显示：E25_0 corrected seedfix aggregate 未保住 object-level AP 且误报高于 E6；E25_1_full、E26_1_full、E27_1_full 虽降低 FP/image 或 FPPI_dark，但 dark-small/tiny/low-contrast object-level AP 明显下降，因此均为 not_candidate。E26_2a/E26_2b 因 class_confusion 来源 CSV 的 split 字段为空，标记 failed_train_split_source_unverified，只保留为失败审计样本。
...
- `S7_1a` gate review 已完成：mAP50-95=0.618546，AP_dark-small=0.490013，AP_tiny=0.538640，AP_low-contrast=0.618347，FP/image=2.562968，FPPI_dark=4.474432，FPPI_low-contrast=2.877285，AP_dark-small_object=0.085120，AP_tiny_object=0.050800，AP_low-contrast_object=0.236658；`FP metrics down = 0/3`，标签为 `not_candidate`。该结果说明当前 UTAH-lite 默认版没有解决高分 FP，且同时损伤 object-level，不进入 `S7_1b`、`S7_6` 或 `S7_7`。
- 当前下一步实验方向切换为 `S7_3a: E6 + Evidential reliability fusion lite (P4/P5-only)`，而不是 `S7_1b`、强模型 `E15` 或 test `E21`。
...
| S7-A | val-only 架构候选孵化阶段 | S7_0、S7_1、S7_3、S7_4、S7_2、S7_5、S7_6、S7_7 | s7_1a_reviewed_next_s7_3a | S7_0 已完成；S7_1a 已完成并判定 not_candidate；下一步转入 S7_3a Evidential reliability fusion lite 的实现与启动准备 |
| S7-B | 论文完整验证阶段 | E24_full、E15/E16/E19、E18_full、E20_full、E21 | blocked_no_valid_candidate | 只有 S7-A 至少产生一个通过 gate 的单项或组合候选后才允许启动 |

当前进度：S6.5 audit-only 收口完成，暂无有效新候选；E6 仍是当前主线基线。`S7_0` 冻结产物已生成；`S7_1a: E6 + UTAH-lite quality-aligned head` 已于 2026-05-29 完成训练、统一验证、object-level、prediction export 和 gate review，但结论为 `not_candidate`，因此不再继续 `S7_1b`。下一步转入 `S7_3a: Evidential reliability fusion lite (P4/P5-only)` 的实现与启动准备。旧 E18/E25_0 多 seed 结果 invalid；只允许使用 2026-05-27 corrected seedfix 后的新训练、验证、prediction export 和 object-level 产物。E25_0/E25_1_full/E26_1_full/E27_1_full 均未形成候选；E26_2a/E26_2b 为 failed audit samples。
...
| UTAH-lite quality-aligned head | S7_1a | done_not_candidate | 2026-05-29 gate review：mAP50-95=0.618546，FP/image=2.562968，FPPI_dark=4.474432，AP_dark-small_object=0.085120；`FP metrics down = 0/3`，不进入 `S7_1b` |
| Evidential reliability fusion lite | S7_3a | next_to_implement_and_launch | 每尺度估计 RGB/IR evidence、uncertainty、conflict map，残差式调制 E6 fusion；当前推荐先做 `P4/P5-only` 版本，把“可靠性有信号”从后处理移回网络内部 |
...
## 2026-05-28 S7-A 落盘进度

- `S7_0` 冻结产物已生成：`results/S7_architecture_incubation/s7_0_freeze_audit_refresh/s7_0_freeze_summary.md` 和 `.json`。
- `S7_1a` UTAH-lite 已落盘为真实训练入口：`scripts/s7_1_train_utah_lite.py`、`scripts/s7_1_val_utah_lite.py`、`scripts/s7_utah_quality_head_core.py`。
- `S7_1a` demo 已接入：`scripts/train/run_dark_small_experiment_demos.sh S7`、`S7_1a`、`S7_1a_val`、`S7_1a_object`、`S7_1a_export`、`S7_1a_gate`；默认双卡高吞吐配置为 `device=0,1`、`batch=96`、`workers=16`。
- `S7_1a` object-level 与 prediction export 已接入 `--validator s7_1`，后续训练完成后可直接执行完整 val/object/export/gate 闭环。
- 已完成 dry-run、Python 编译和 CPU 建图 smoke；建图确认 `UtahLiteDetect=True`，质量分支 `cv4` 三尺度、42 个 quality tensors。
- 2026-05-29：`S7_1a` 已完成 full 闭环。gate review 产物位于 `results/S7_architecture_incubation/s7_1a_utah_lite_a06_b04_gate_review/`，标签 `not_candidate`；由于 FP 和 object-level 同时劣化，当前明确不启动 `S7_1b`。
- 当前下一步方向切换为 `S7_3a`。仓库内尚未落盘 `S7_3a` 的真实训练/验证/demo 入口，因此下一动作是补齐实现与 manifest，再由用户按宿主命令行启动。
- 后台启动脚本为：`scripts/train/launch_s7_1a_utah_lite_dual4090_background.sh`。脚本会写入 `results/S7_architecture_incubation/logs/`，同时生成 PID 文件 `results/S7_architecture_incubation/logs/s7_1a_utah_lite_a06_b04.pid`。

## Demo 脚本

- 全量编号 demo 入口：`scripts/train/run_dark_small_experiment_demos.sh`，覆盖 E0_1-E24_3，默认 dry-run；执行时会打印 `Sx / Ex` 阶段前缀。
- 当前 S7-A demo：`scripts/train/run_dark_small_experiment_demos.sh S7`，会展开 `S7_0` 与 `S7_1a`；训练完成后执行 `S7_1a_val S7_1a_object S7_1a_export S7_1a_gate` 补齐必需产物。
...
- 当前 E6 主线快捷 demo：`scripts/train/run_e6_mainline_demos.sh`；执行时同样打印 `Sx / Ex` 阶段前缀。
